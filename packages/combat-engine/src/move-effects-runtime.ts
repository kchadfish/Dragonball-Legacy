...
// Runtime effect data is validated independently of its narrowed in-memory type.
...
export interface MoveEffectRuntimeContext {
  ...
  /** Source selectors whose CONSTANT activation was unavailable at this resolution boundary. */
  readonly activationUnavailableSelectors?: readonly MoveSelectorCondition[];
  /** Move IDs selected for declarative suppression effects awaiting attack resumption. */
...
export interface DamageModifierTransformerApplication {
  readonly target: "self" | "opponent";
  readonly sourceCombatantId: CombatantState["id"];
  readonly sourceDefinitionId: MoveDefinition["id"];
  readonly effectIndex: number;
  readonly multiplier: number;
  readonly scope: "current-action";
}

export interface ResourceModifierTransformerApplication {
  readonly target: "self" | "opponent";
  readonly sourceCombatantId: CombatantState["id"];
  readonly sourceDefinitionId: MoveDefinition["id"];
  readonly effectIndex: number;
...
export interface CostModifierTransformerApplication {
  readonly target: "self" | "opponent";
  readonly sourceCombatantId: CombatantState["id"];
  readonly sourceDefinitionId: MoveDefinition["id"];
  readonly effectIndex: number;
  readonly multiplier: number;
  readonly scope: "next-cost-modification";
}
...
export interface TransformationReversionApplication {
  readonly target: "self" | "opponent";
  readonly sourceDefinitionId: MoveDefinition["id"];
  readonly sourceEffectIndex: number;
}

export interface StatComparisonApplication {
  readonly target: "self" | "opponent";
  readonly sourceDefinitionId: MoveDefinition["id"];
  readonly sourceEffectIndex: number;
...
export interface TransformationActionApplication {
  readonly target: "self" | "opponent";
  readonly sourceDefinitionId: MoveDefinition["id"];
  readonly sourceEffectIndex: number;
  readonly useLimit?: { readonly scope: "combat"; readonly count: number };
  readonly phase?: "action" | "end";
...
export interface DeferredMoveApplication {
  readonly target: "self";
  readonly sourceDefinitionId: MoveDefinition["id"];
  readonly effectIndex: number;
  readonly performAfterTurns: number;
  readonly damageOverridePercent?: number;
  readonly cancellation: { readonly actor: "opponent"; readonly result: "successful" };
...
export interface ActivationApplication {
  readonly trigger:
    "action" | "before-attack-roll" | "on-success" | "start-combat" | "on-roll-result";
  readonly scope?: {
  ...
  readonly selectionKey?: string;
  /** Selects every eligible constant in a keyed activation group. */
...
export interface EndFloatingEffectApplication {
  readonly target: "self" | "opponent";
  readonly sourceEffectIndex: number;
}
...
export interface MoveEffectReplacementApplication {
  readonly sourceCombatantId: CombatantState["id"];
  readonly sourceDefinitionId: MoveDefinition["id"];
  readonly sourceEffectIndex: number;
  readonly target: "self" | "opponent";
  readonly targetMoveId: MoveDefinition["id"];
  readonly replacement: Extract<EffectDefinition, { readonly type: "modify-resource" }>;
}
...
export interface DeactivationNegationApplication {
  readonly target: "self" | "opponent";
  readonly selector?: MoveSelectorCondition;
  readonly optional: boolean;
  readonly sourceDefinitionId: MoveDefinition["id"];
...
export interface DamageReductionCostModificationApplication {
  readonly target: "self" | "opponent";
  readonly sourceCombatantId: CombatantState["id"];
  readonly sourceDefinitionId: MoveDefinition["id"];
  readonly sourceEffectIndex: number;
...
export interface TransformationRollRequirementApplication {
  readonly target: "self" | "opponent";
  readonly sourceDefinitionId: MoveDefinition["id"];
  readonly sourceEffectIndex: number;
...
export interface CostModification {
  ...
  readonly operation: "add" | "set";
  readonly amount: number;
  readonly amountExpression?: Extract<NumericExpression, { readonly type: "next-move-ki-cost" }>;
  readonly minimum?: number;
  readonly maximum?: number;
  readonly selector?: MoveSelectorCondition;
  readonly scope: "next-action" | "next-actions" | "next-turn";
  readonly remaining?: number;
}
...
export interface ResolutionImmunityOverrideApplication {
  readonly target: "self" | "opponent";
...
export interface CombatOutcomeModifierApplication {
  readonly target: "self" | "opponent";
  readonly outcome: "break" | "sever" | "stun";
  readonly multiplier: number;
  readonly selector: MoveSelectorCondition;
  readonly scope: "next-action" | "next-turn";
  readonly sourceDefinitionId: MoveId;
  readonly sourceEffectIndex: number;
}
...
export interface ResourceModificationPreventionApplication {
  ...
  /** The first owner turn on which a next-turn prevention may apply. */
  readonly availableFromTurn?: number;
  readonly duration?: LockApplication["duration"];
}
...
const statComparisonMatches = (
  ...
  const rightCombatant = condition.right === "self" ? context.self : context.opponent;
  if (
    condition.stat === "dexterity" &&
    condition.comparison === "higher-than" &&
    context.activeEffects?.some(
      (effect) =>
        effect.type === "set-stat-comparison" &&
        effect.leftCombatantId === leftCombatant.id &&
        effect.rightCombatantId === rightCombatant.id &&
        effect.stat === condition.stat &&
        effect.comparison === condition.comparison,
    ) === true
  )
    return true;
...
const rollModificationMatches = (
  ...
  return (
    ...
    runtimeValue(event.excludeSource) === condition.excludeSource &&
...
const stoppedHitFractionMatches = (
  ...
  if (runtimeValue(condition.comparison) !== "more-than-half") return false;
...
const conditionCombatStateMatches = (
  ...
  const combatant = condition.subject === "self" ? context.self : context.opponent;
  return runtimeValue(condition.state) !== "transformed" || combatant.transformation !== undefined;
};
...
const conditionStatusMatches = (
  ...
  const combatant = condition.subject === "self" ? context.self : context.opponent;
  return (
    runtimeValue(condition.state) === "active" &&
  ...
  const active = (context.activeEffects ?? []).some((effect) => {
    ...
    const sourceMove =
      effect.type === "active-constant"
        ? (effect.replacement?.sourceMoveSnapshot ?? context.moves.get(effect.sourceDefinitionId))
        : context.moves.get(effect.sourceDefinitionId);
...
const withoutSourceText = (key: string, value: unknown) =>
  key === "sourceText" ? undefined : value;

const activationUnavailableMatches = (
  condition: Extract<RuntimeCondition, { readonly type: "activation-unavailable" }>,
  context: MoveEffectRuntimeContext,
) =>
  condition.selector.subject === "source" &&
  (context.activationUnavailableSelectors ?? []).some(
    (selector) =>
      JSON.stringify(selector, withoutSourceText) ===
      JSON.stringify(condition.selector, withoutSourceText),
  );
...
const resourceChangeMatches = (
  ...
  if (condition.timing === "current-event") {
  ...
  const subjectId = condition.subject === "self" ? context.self.id : context.opponent.id;
  const minimumTurn =
    condition.timing === "last-turn"
      ? context.turnNumber - 1
      : context.turnNumber - (condition.turns ?? 0);
  return (context.actionHistory ?? []).some((action) => {
    ...
    return (
      resourceChanges?.some(
        (change) =>
          change.affectedCombatantId === subjectId &&
          change.turnNumber >= minimumTurn &&
          change.turnNumber < context.turnNumber &&
          change.resource === condition.resource &&
          change.operation === condition.operation &&
          (condition.cause === undefined || condition.cause === change.cause) &&
          (condition.sourceStyleId === undefined ||
            condition.sourceStyleId === change.sourceStyleId),
      ) === true
    );
  });
};
...
const locationMatches = (
  ...
  const combatant = condition.subject === "self" ? context.self : context.opponent;
  return (
    runtimeValue(condition.state) === "planet-has-dragon-balls" &&
    combatant.planetHasDragonBalls === true
  );
};
...
const moveModificationMatches = (
  ...
  return [...(context.activeEffects ?? [])].some((activeEffect) => {
    ...
    const sourceMove =
      activeEffect.replacement?.sourceMoveSnapshot ??
      context.moves.get(activeEffect.sourceDefinitionId);
    return (
...
const runtimeConditionHandlers = {
  ...
  "activation-unavailable": (condition, context) =>
    activationUnavailableMatches(
      condition as Extract<RuntimeCondition, { readonly type: "activation-unavailable" }>,
      context,
    ),
...
const combatLimitedEffectAlreadyUsed = (
  effect: EffectDefinition,
  context: MoveEffectRuntimeContext,
) => {
  if (
  ...
  return (
    (context.activeEffects?.filter(
      (activeEffect) =>
        (activeEffect.type === "floating-effect" || activeEffect.type === "suppress") &&
        activeEffect.sourceCombatantId === context.self.id &&
        activeEffect.sourceDefinitionId === context.sourceDefinitionId &&
        activeEffect.sourceEffectIndex === context.sourceEffectIndex,
    ).length ?? 0) >= count
  );
};
...
const effectMatches = (effect: EffectDefinition, context: MoveEffectRuntimeContext) =>
  ...
  !combatLimitedEffectAlreadyUsed(effect, context) &&
...
export const adjustedMoveDamage = (
  ...
  (move.effects ?? []).reduce((damage, effect) => {
    ...
    if (effect.cap !== undefined && effect.percent === undefined) {
      ...
      return runtimeValue(effect.cap.type) === "maximum"
        ? Math.min(damage, cap)
        : Math.max(damage, cap);
    }
...
const damageModificationLifecycle = (
  ...
  return {
    scope: scope === undefined || scope === "next-turn" ? undefined : scope,
    remaining,
    duration,
    cap,
    nextTurn,
...
const activationCostFor = (effect: ActivationCostCarrier, context: MoveEffectRuntimeContext) => {
...
const resolvedDamageModification = (
  ...
  const activationCost = activationCostFor(effect, context);
...
const moveClassificationEffectChanges = (
  ...[effect, move, context, target, trigger, effectIndex]: [
    effect: Extract<EffectDefinition, { readonly type: "modify-move-classification" }>,
    move: MoveDefinition,
    context: MoveEffectRuntimeContext,
    target: "self" | "opponent",
    trigger: string,
      ...
      moveClassifications: [
        {
          ...
          classification: { type: "replace-style-id", styleId: effect.setStyleId },
  ...
  if (turns === undefined || !Number.isInteger(turns) || turns < 1) return emptyEffectChanges();
  return {
    ...emptyEffectChanges(),
    moveClassifications: [
      {
        ...
        effectIndex,
        selector: effect.selector,
        classification: "replace-declared-style",
        duration: { type: "turns", turns },
      },
    ],
  };
};
...
const suppressionUseLimit = (
  ...
  const count =
    typeof effect.useLimit.count === "number"
      ? effect.useLimit.count
      : numeric(effect.useLimit.count, context);
  return count === undefined || !Number.isInteger(count) || count < 1
    ? undefined
    : { scope: effect.useLimit.scope, count };
};
...
const suppressionEffectChanges = (
  ...
  const activationCost = activationCostFor(effect, context);
  const useLimit = suppressionUseLimit(effect, context);
  if (
    (effect.activationCost !== undefined && activationCost === undefined) ||
    (effect.useLimit !== undefined && useLimit === undefined)
  )
    return emptyEffectChanges();
  return {
    ...emptyEffectChanges(),
    suppressions: [
      {
        ...
        ...(context.selectedSuppressionMoveIds?.[effectIndex] === undefined
          ? {}
          : { selectedMoveId: context.selectedSuppressionMoveIds[effectIndex] }),
...
const suppressRequirementEffectChanges = (
  effect: Extract<EffectDefinition, { readonly type: "suppress-requirement" }>,
  context: MoveEffectRuntimeContext,
  target: "self" | "opponent",
  effectIndex: number,
): EffectChanges => {
...
const emptyEffectChanges = () => ({
  ...
  damageModifierTransformers: [] as DamageModifierTransformerApplication[],
  resourceModifierTransformers: [] as ResourceModifierTransformerApplication[],
  costModifierTransformers: [] as CostModifierTransformerApplication[],
  ...
  deferredMoves: [] as DeferredMoveApplication[],
  ...
  statComparisons: [] as StatComparisonApplication[],
  transformationActions: [] as TransformationActionApplication[],
  floatingEffects: [] as FloatingEffectApplication[],
  endFloatingEffects: [] as EndFloatingEffectApplication[],
  moveUsePreventions: [] as MoveUsePreventionApplication[],
  remainingUseModifications: [] as RemainingUseModificationApplication[],
  moveEffectReplacements: [] as MoveEffectReplacementApplication[],
  statusPreventions: [] as StatusPreventionApplication[],
  negations: [] as NegationApplication[],
  deactivationNegations: [] as DeactivationNegationApplication[],
  ...
  resolutionImmunityOverrides: [] as ResolutionImmunityOverrideApplication[],
  damageReductionCostModifications: [] as DamageReductionCostModificationApplication[],
  contests: [] as ContestApplication[],
  destructionMasteries: [] as DestructionMasteryApplication[],
  transformationRollRequirements: [] as TransformationRollRequirementApplication[],
  combatResultOverrides: [] as CombatResultOverrideApplication[],
  combatOutcomeModifiers: [] as CombatOutcomeModifierApplication[],
  ...
  moveClassifications: [] as MoveClassificationApplication[],
  rerolls: [] as RerollApplication[],
  requireAllDiceSuccess: false,
});
...
const statusApplicationChanges = (
  ...[statusId, durationDefinition, selector, move, context, target]: [
  ...
  } else if (durationDefinition?.type === "until-turn-start-roll-threshold") {
    const value = numeric(durationDefinition.value, context);
    if (value === undefined) return emptyEffectChanges();
    duration = {
      type: "until-turn-start-roll-threshold",
      combatantId: durationDefinition.subject === "self" ? context.self.id : context.opponent.id,
      dice: durationDefinition.dice,
      sides: durationDefinition.sides,
      comparison: durationDefinition.comparison,
      value,
      remainingIgnoredChecks:
        (durationDefinition.ignoreFirstCheck ? 1 : 0) + (durationDefinition.startAfterTurns ?? 0),
    };
  } else if (durationDefinition === undefined) {
    duration = { type: "turns", ownerCombatantId, remaining: 1 };
  } else {
    throw new Error(`Unsupported converted APPLY STATUS duration: ${durationDefinition.type}`);
  }
  return {
    ...emptyEffectChanges(),
    storedRollRequests: [],
    ...
    rollDefinitions: [],
    rollResultOverrides: [],
    resolutionImmunityOverrides: [],
    damageReductionCostModifications: [],
    contests: [],
    destructionMasteries: [],
    transformationRollRequirements: [],
...
const combatOutcomeEffectChanges = (
  ...
  if (
    move.id === "move-kiihakai-ki-barbs" &&
    effectIndex === 2 &&
    effect.scope?.type === "next-turn" &&
    effect.selector !== undefined &&
...
const transformationReversionEffectChanges = (
  effect: Extract<EffectDefinition, { readonly type: "revert-transformation" }>,
  move: MoveDefinition,
...
const statComparisonEffectChanges = (
  effect: Extract<EffectDefinition, { readonly type: "set-stat-comparison" }>,
  move: MoveDefinition,
  context: MoveEffectRuntimeContext,
  target: "self" | "opponent",
  effectIndex: number,
  ...
  const turns =
    effect.duration?.type === "turns" ? numeric(effect.duration.turns, context) : undefined;
  return turns === undefined || turns < 1
    ? emptyEffectChanges()
    : {
        ...emptyEffectChanges(),
        statComparisons: [
...
const transformationActionEffectChanges = (
  effect: Extract<EffectDefinition, { readonly type: "grant-transformation-action" }>,
  move: MoveDefinition,
  target: "self" | "opponent",
  effectIndex: number,
): EffectChanges => ({
  ...
  transformationActions: [
    {
      ...
      ...(effect.useLimit?.scope === "combat" && typeof effect.useLimit.count === "number"
        ? { useLimit: { scope: "combat" as const, count: effect.useLimit.count } }
        : {}),
    },
  ],
});

const forceTransformationEffectChanges = (
  effect: Extract<EffectDefinition, { readonly type: "force-transformation" }>,
  move: MoveDefinition,
  target: "self" | "opponent",
  effectIndex: number,
): EffectChanges => ({
  ...
  transformationActions: [
    {
      ...
      sourceEffectIndex: effectIndex,
      phase: "end",
      targetTransformation: effect.targetTransformation,
      optional: effect.required === false,
    },
  ],
});
  ...
  const retainsTargetRelation = effect.effects?.some((nestedEffect) =>
    nestedEffect.conditions?.some(
      (condition) =>
        ...
        condition.subject === "source" &&
        runtimeValue(condition.relation) === "same-as-source-effect-target",
    ),
  );
...
const endFloatingEffectChanges = (
  _effect: Extract<EffectDefinition, { readonly type: "end-floating-effect" }>,
  _context: MoveEffectRuntimeContext,
  target: "self" | "opponent",
  effectIndex: number,
): EffectChanges => ({
  ...emptyEffectChanges(),
  endFloatingEffects: [{ target, sourceEffectIndex: effectIndex }],
});
...
const selectMoveByStoredRollEffectChanges = (
  ...
  if (
    target !== "self" ||
    runtimeValue(effect.subject) !== "self" ||
    effect.selector.subject !== "source" ||
    runtimeValue(effect.ordering) !== "character-sheet-top-to-bottom" ||
    runtimeValue(effect.reindex) !== "on-moveset-change"
  )
    return emptyEffectChanges();
  return {
    ...emptyEffectChanges(),
...
/* eslint-disable sonarjs/cognitive-complexity -- this adapter resolves the exact declarative extra-action variant into one deterministic runtime change. */
const extraActionEffectChanges = (
  ...[effect, move, context, target, , effectIndex]: [
...
type EffectChanges = Omit<
  ...
  "resourceActionModifiers" | "resourceCostModifications" | "moveRemovals" | "deferredMoves"
> & {
  ...
  readonly damageModifierTransformers?: readonly DamageModifierTransformerApplication[];
  readonly resourceModifierTransformers?: readonly ResourceModifierTransformerApplication[];
  readonly costModifierTransformers?: readonly CostModifierTransformerApplication[];
  readonly moveRemovals?: readonly MoveRemovalApplication[];
  readonly deferredMoves?: readonly DeferredMoveApplication[];
};
...
const resourceEffectAmount = (
  ...
  if (effect.amount.type === "triggering-resource-change") {
    const resourceChange = context.resourceChange;
    return resourceChange !== undefined &&
      resourceChange.resource === effect.amount.resource &&
      resourceChange.operation === "lose"
...
const resourceChangeFromEffect = (
  ...[effect, move, context, target, amount, cap, activationCost, useLimit, effectIndex]: [
    effect: Extract<EffectDefinition, { readonly type: "modify-resource" }>,
    move: MoveDefinition,
    context: MoveEffectRuntimeContext,
    target: "self" | "opponent",
...
const costModifierTransformerChanges = (
  effect: Extract<EffectDefinition, { readonly type: "modify-cost-modifier" }>,
  move: MoveDefinition,
  context: MoveEffectRuntimeContext,
  target: "self" | "opponent",
  effectIndex: number,
): EffectChanges => {
  const multiplier = numeric(effect.multiplier, context);
        ...
        costModifierTransformers: [
          {
            ...
            effectIndex,
            multiplier,
            scope: "next-cost-modification",
          },
        ],
      };
};

const damageModifierTransformerChanges = (
  effect: Extract<EffectDefinition, { readonly type: "modify-damage-modifier" }>,
  move: MoveDefinition,
  context: MoveEffectRuntimeContext,
  target: "self" | "opponent",
  effectIndex: number,
...
const resourceModifierTransformerChanges = (
  effect: Extract<EffectDefinition, { readonly type: "modify-resource-modifier" }>,
  move: MoveDefinition,
  context: MoveEffectRuntimeContext,
  target: "self" | "opponent",
  effectIndex: number,
  ...
  const multiplier = numeric(effect.multiplier, context);
  const cap = effect.cap === undefined ? undefined : numeric(effect.cap.value, context, move.id);
  return multiplier === undefined || (effect.cap !== undefined && cap === undefined)
    ? emptyEffectChanges()
    : {
        ...emptyEffectChanges(),
        resourceModifierTransformers: [
          {
            ...
            multiplier,
            scope: "next-turn",
            ...(cap === undefined ? {} : { cap: { type: "maximum" as const, value: cap } }),
            selector: effect.selector,
          },
        ],
      };
...
const resourceModificationPreventionEffectChanges = (
  ...
  const availableFromTurn = effect.scope?.type === "next-turn" ? context.turnNumber + 1 : undefined;
  if (effect.scope?.type === "next-turn") {
    duration = { type: "turns", remaining: 1 };
  } else if (effect.duration?.type === "turns") {
    const turns = numeric(effect.duration.turns, context);
    ...
    resourceModificationPreventions: [
      {
        ...
        ...(availableFromTurn === undefined ? {} : { availableFromTurn }),
        duration,
      },
    ],
  };
...
const currentActionCostEffectChanges = (
  ...[effect, move, context, target, values, activationCost, effectIndex]: [
    effect: Extract<EffectDefinition, { readonly type: "modify-cost" }>,
    move: MoveDefinition,
    context: MoveEffectRuntimeContext,
    target: "self" | "opponent",
...
const deferredNextMoveCostDefinition = (
  effect: Extract<EffectDefinition, { readonly type: "modify-cost" }>,
):
  | {
      readonly amount: Extract<NumericExpression, { readonly type: "next-move-ki-cost" }>;
      readonly selector: MoveSelectorCondition;
    }
  // eslint-disable-next-line complexity -- Effect translation intentionally keeps related declarative branches together.
  | undefined => {
  ...
  if (
    ...
    return undefined;
  return { amount: effect.amount, selector };
};

const deferredNextMoveCostValues = (
  effect: Extract<EffectDefinition, { readonly type: "modify-cost" }>,
  move: MoveDefinition,
): ResolvedCostModificationValues | undefined => {
  if (
    ...
    return undefined;
  return { amount: move.mechanics.kiCost.value };
};
...
const costModificationEffectChanges = (
  ...
  const deferredNextMoveCost = deferredNextMoveCostDefinition(effect);
  const values =
    deferredNextMoveCost === undefined
      ? resolvedCostModificationValues(effect, context, move.id)
      : deferredNextMoveCostValues(effect, move);
  ...
  const scope = effect.scope?.type;
  if (deferredNextMoveCost !== undefined) {
    return {
      ...emptyEffectChanges(),
      costModifications: [
        {
          ...
          operation: effect.operation,
          ...values,
          amountExpression: deferredNextMoveCost.amount,
          selector: { ...deferredNextMoveCost.selector, ids: [move.id] },
          scope: "next-turn",
        },
      ],
    };
...
const actionRestrictionDuration = (
  effect: Extract<EffectDefinition, { readonly type: "skip-action" }>,
  context: MoveEffectRuntimeContext,
): ActionRestrictionApplication["duration"] => {
  if (effect.duration?.type !== "until-turn-start-roll-threshold") return undefined;
  const value = numeric(effect.duration.value, context);
  if (value === undefined) return undefined;
  return {
    type: "until-turn-start-roll-threshold",
    subject: effect.duration.subject,
    dice: effect.duration.dice,
    sides: effect.duration.sides,
    comparison: effect.duration.comparison,
    value,
    remainingIgnoredChecks: effect.duration.startAfterTurns ?? 0,
  };
};
...
const actionRestrictionEffectChanges = (
  ...
  if (
    ...
    return {
      ...emptyEffectChanges(),
      actionRestrictions: [{ target, remainingTurns: 1, effectIndex }],
    };
  const remainingTurns = actionRestrictionRemainingTurns(effect, context);
  const duration = actionRestrictionDuration(effect, context);
  if (
    (remainingTurns === undefined && duration === undefined) ||
    (remainingTurns !== undefined && (!Number.isInteger(remainingTurns) || remainingTurns < 1))
  )
    return emptyEffectChanges();
  return {
    ...emptyEffectChanges(),
...
const deactivationEffectChanges = (
  ...
  if (
    activationCost !== undefined &&
    (activationCost.amount === undefined ||
      (effect.activationCost?.minimum !== undefined && activationCost.minimum === undefined))
  )
    return emptyEffectChanges();
  return {
    ...emptyEffectChanges(),
...
const hasLinkedDelayedDeactivation = (
  ...
  move.effects?.some(
    (candidate) =>
      candidate.type === "delayed-deactivate" &&
      candidate.trigger === effect.trigger &&
      candidate.target === target &&
      runtimeValue(candidate.affectedType) === "skill" &&
      candidate.selectionKey === effect.selectionKey &&
      candidate.turnsAfter >= 1 &&
      JSON.stringify(candidate.conditions ?? []) === JSON.stringify(effect.conditions ?? []),
  ) === true;

const invalidActivationSelection = ({
  effect,
  target,
  selectionLimit,
  activationCost,
  ...
  readonly effect: Extract<EffectDefinition, { readonly type: "activate" }>;
  readonly target: "self" | "opponent";
  readonly selectionLimit: number | undefined;
  readonly activationCost: number | undefined;
  readonly activationMinimum: number | undefined;
  // eslint-disable-next-line complexity -- Effect translation intentionally keeps related declarative branches together.
}) => {
  if (
...
const activationEffectChanges = (
  ...
  if (
    ...
    return emptyEffectChanges();
  const linkedDelayedDeactivation = hasLinkedDelayedDeactivation(effect, move, target);
    ...
    activations: [
      {
        ...
        ...(effect.asIf === undefined ? {} : { asIf: effect.asIf }),
        ...(effect.selectionKey === undefined ? {} : { selectionKey: effect.selectionKey }),
        ...(linkedDelayedDeactivation ? { selectionMode: "all" as const } : {}),
...
const reactivationEffectChanges = (
  effect: Extract<
    EffectDefinition,
  ...
  const selector =
    effect.type === "reactivate-deactivated-constant-skill"
      ...
      : {
          ...
          sourceText: "one of your CONSTANT Skills" as const,
        };
  return {
    ...emptyEffectChanges(),
    activations: [
      {
        ...
        ...(effect.type === "reactivate-recent-skill"
          ? { reactivationTiming: "last-turn" as const }
          : {}),
        ...(effect.type === "reactivate-recent-skill"
          ? { activationCost: { amount: effect.payment.amount } }
          : {}),
        sourceDefinitionId: move.id,
        sourceText: effect.sourceText,
      },
...
const protectedActivationEffectChanges = (
  ...[effect, move, sourceCombatantId, target, trigger, effectIndex]: [
    effect: Extract<EffectDefinition, { readonly type: "activate-protected-constant" }>,
    move: MoveDefinition,
    sourceCombatantId: CombatantId,
    target: "self" | "opponent",
    trigger: string,
  ...
  activations: [
    {
      ...
      sourceCombatantId,
      activationCost: { amount: effect.payment.amount },
      deactivationProtectionTurns: effect.protectionDuration.turns,
      sourceDefinitionId: move.id,
      sourceText: effect.sourceText,
    },
  ],
        ...
        remainingUseModifications: [
          {
            ...
            amount,
            selector: effect.selector,
            ...(context.selectedMoveIds?.[effectIndex] === undefined
              ? {}
              : { selectedMoveId: context.selectedMoveIds[effectIndex] }),
          },
        ],
      };
};

const moveEffectReplacementChanges = (
  effect: Extract<EffectDefinition, { readonly type: "replace-move-effect" }>,
  move: MoveDefinition,
  context: MoveEffectRuntimeContext,
  target: "self" | "opponent",
  effectIndex: number,
  ...
  const selectedMoveId =
    context.selectedMoveIds?.[effectIndex] ??
    (effect.activationGroup === undefined
      ? undefined
      : Object.entries(context.selectedMoveIds ?? {}).find(([selectedEffectIndex]) => {
          const selectedEffect = move.effects?.[Number(selectedEffectIndex)];
          return (
            selectedEffect?.type === "modify-remaining-uses" &&
            selectedEffect.activationGroup === effect.activationGroup
          );
        })?.[1]);
  const selectedMove = selectedMoveId === undefined ? undefined : context.moves.get(selectedMoveId);
  if (
    target !== "self" ||
    selectedMoveId === undefined ||
    selectedMove === undefined ||
    !matchesMoveSelector(selectedMove, effect.selector) ||
    runtimeValue(effect.replacement.trigger) !== "on-resource-drain" ||
    runtimeValue(effect.replacement.target) !== "self" ||
    runtimeValue(effect.replacement.type) !== "modify-resource" ||
    runtimeValue(effect.replacement.resource) !== "ki" ||
    runtimeValue(effect.replacement.operation) !== "gain"
  )
    return emptyEffectChanges();
  return {
    ...emptyEffectChanges(),
...
/* eslint-disable sonarjs/cognitive-complexity -- this adapter preserves roll-transformer payment and duration boundaries. */
...
const combatOutcomeModifierEffectChanges = (
  effect: Extract<EffectDefinition, { readonly type: "modify-combat-outcome" }>,
  move: MoveDefinition,
  context: MoveEffectRuntimeContext,
  target: "self" | "opponent",
  effectIndex: number,
): EffectChanges => {
  const multiplier = numeric(effect.multiplier, context, move.id);
...
const preventLowRollStopEffectChanges = (
  effect: Extract<EffectDefinition, { readonly type: "prevent-low-roll-stop" }>,
  context: MoveEffectRuntimeContext,
  target: "self" | "opponent",
): EffectChanges => {
  const value = numeric(effect.value, context);
...
const resolutionImmunityOverrideEffectChanges = (
  effect: Extract<EffectDefinition, { readonly type: "override-resolution-immunity" }>,
  target: "self" | "opponent",
): EffectChanges => ({
  ...emptyEffectChanges(),
  resolutionImmunityOverrides: [{ target, resolution: effect.resolution }],
});

const damageReductionCostEffectChanges = (
  effect: Extract<EffectDefinition, { readonly type: "modify-damage-reduction-cost" }>,
  move: MoveDefinition,
  context: MoveEffectRuntimeContext,
  target: "self" | "opponent",
  effectIndex: number,
): EffectChanges => {
  const amount = numeric(effect.amount, context, move.id);
  if (
    amount === undefined ||
    effect.resource !== "ki" ||
...
const contestEffectChanges = (
  effect: Extract<EffectDefinition, { readonly type: "resolve-contest" }>,
  move: MoveDefinition,
  context: MoveEffectRuntimeContext,
  target: "self" | "opponent",
  effectIndex: number,
): EffectChanges => {
  // Participants are expanded to both relative targets by the dispatcher, but
  // the contest itself is one shared resolution, not one contest per target.
  if (target !== "self") return emptyEffectChanges();
  const penaltyAmount = numeric(effect.penalty.amount, context, move.id);
  if (penaltyAmount === undefined) return emptyEffectChanges();
  return {
    ...emptyEffectChanges(),
    contests: [
      {
        ...
        sourceDefinitionId: move.id,
        sourceEffectIndex: effectIndex,
        dice: effect.rolls.dice,
        sides: effect.rolls.sides,
        repetitions: effect.rolls.repetitions,
        defaultThreshold: effect.qualifyingThreshold.default,
        selfPowerHigherThreshold: effect.qualifyingThreshold.whenSelfPowerHigher,
        penaltyAmount,
        loser: effect.loser,
        tie: effect.tie,
      },
...
const exchangeConstantSkillEffectChanges = (
  effect: Extract<EffectDefinition, { readonly type: "exchange-constant-skill" }>,
  move: MoveDefinition,
): EffectChanges => ({
  ...emptyEffectChanges(),
  deactivations: [
...
const destructionMasteryEffectChanges = (
  effect: Extract<EffectDefinition, { readonly type: "grant-destruction-mastery" }>,
  move: MoveDefinition,
  effectIndex: number,
): EffectChanges => ({
  ...emptyEffectChanges(),
  destructionMasteries: [
    {
      target: "self",
      advancedAttack: effect.advancedAttack,
      signatureTechnique: effect.signatureTechnique,
      naturalDefenseStopPreventionAtMost: effect.naturalDefenseStopPreventionAtMost,
      zeroCostSignatureUses: effect.zeroCostSignatureUses,
      targetsInterferers: true,
      damageBonusAfterOpponentInterferencePercent:
        effect.damageBonusAfterOpponentInterferencePercent,
      sourceDefinitionId: move.id,
      sourceEffectIndex: effectIndex,
    },
  ],
...
const transformationRollRequirementEffectChanges = (
  effect: Extract<EffectDefinition, { readonly type: "require-transformation-roll" }>,
  move: MoveDefinition,
  target: "self" | "opponent",
  effectIndex: number,
): EffectChanges => ({
  ...
  transformationRollRequirements: [
    {
      ...
      sourceDefinitionId: move.id,
      sourceEffectIndex: effectIndex,
      ignoreTransformationDice: effect.ignoreTransformationDice,
    },
  ],
});

const stopAttackByDeactivationEffectChanges = (
  effect: Extract<EffectDefinition, { readonly type: "stop-attack-by-deactivation" }>,
  context: MoveEffectRuntimeContext,
): EffectChanges =>
  context.triggeringMove === undefined ||
  !matchesMoveSelector(context.triggeringMove, effect.attack)
    ? emptyEffectChanges()
    : {
        ...emptyEffectChanges(),
        combatResultOverrides: [
...
const substituteDefenseEffectChanges = (
  effect: Extract<EffectDefinition, { readonly type: "substitute-defense" }>,
  context: MoveEffectRuntimeContext,
): EffectChanges => {
  if (
    context.triggeringMove === undefined ||
    ...
    return emptyEffectChanges();
  const amount = numeric(effect.payment.amount, context);
  return amount === undefined
    ? emptyEffectChanges()
    : {
        ...emptyEffectChanges(),
...
const triggeredEffectHandlers: Partial<Record<EffectDefinition["type"], TriggeredEffectHandler>> = {
  activate: (
    ...[effect, move, context, target, trigger, effectIndex]: Parameters<TriggeredEffectHandler>
  ) =>
  ...
  "roll-and-store": (
    ...[effect, move, context, target, , effectIndex]: Parameters<TriggeredEffectHandler>
  ) =>
  ...
  "create-floating-effect": (
    ...[effect, , context, target, , effectIndex]: Parameters<TriggeredEffectHandler>
  ) =>
  ...
  "end-floating-effect": (
    ...
    endFloatingEffectChanges(
      effect as Extract<EffectDefinition, { readonly type: "end-floating-effect" }>,
      context,
      target,
      effectIndex,
    ),
  ...
  "modify-damage-modifier": (
    ...
    damageModifierTransformerChanges(
      effect as Extract<EffectDefinition, { readonly type: "modify-damage-modifier" }>,
      move,
      context,
      target,
      effectIndex,
  ...
  "modify-move-classification": (
    ...
    moveClassificationEffectChanges(
      effect as Extract<EffectDefinition, { readonly type: "modify-move-classification" }>,
      move,
      context,
      target,
      trigger,
  ...
  suppress: (
    ...[effect, , context, target, trigger, effectIndex]: Parameters<TriggeredEffectHandler>
  ) =>
  ...
  "modify-cost-modifier": (
    ...
    costModifierTransformerChanges(
      effect as Extract<EffectDefinition, { readonly type: "modify-cost-modifier" }>,
      move,
      context,
      target,
      effectIndex,
  ...
  "modify-resource-modifier": (
    ...
    resourceModifierTransformerChanges(
      effect as Extract<EffectDefinition, { readonly type: "modify-resource-modifier" }>,
      move,
      context,
      target,
      effectIndex,
  ...
  "replace-move-effect": (
    ...
    moveEffectReplacementChanges(
      effect as Extract<EffectDefinition, { readonly type: "replace-move-effect" }>,
      move,
      context,
      target,
      effectIndex,
  ...
  "defer-move": (
    ...[effect, move, , target, , effectIndex]: Parameters<TriggeredEffectHandler>
  ) => {
    const deferred = effect as Extract<EffectDefinition, { readonly type: "defer-move" }>;
    return {
      ...emptyEffectChanges(),
      deferredMoves: [
        {
          target: target as "self",
          sourceDefinitionId: move.id,
          effectIndex,
          performAfterTurns: deferred.performAfterTurns,
          ...(deferred.damage?.percent.type === "literal"
            ? { damageOverridePercent: deferred.damage.percent.value }
            : {}),
          cancellation: deferred.cancellation,
          ...(deferred.onCancellation === undefined
            ? {}
            : { onCancellation: { affectedType: "attack" as const, duration: "combat" as const } }),
        },
      ],
    };
  },
  ...
  "skip-action": (
    ...
    actionRestrictionEffectChanges(
      ...
      target,
      effectIndex,
      trigger === "action-phase" ? trigger : undefined,
    ),
  "stop-attack-by-deactivation": (effect, _move, context) =>
    stopAttackByDeactivationEffectChanges(
      effect as Extract<EffectDefinition, { readonly type: "stop-attack-by-deactivation" }>,
      context,
    ),
  "substitute-defense": (effect, _move, context) =>
    substituteDefenseEffectChanges(
      effect as Extract<EffectDefinition, { readonly type: "substitute-defense" }>,
      context,
    ),
  ...
  "override-skill-activation-prevention": () => emptyEffectChanges(),
  ...
  "modify-combat-outcome": (
    ...
    combatOutcomeModifierEffectChanges(
      effect as Extract<EffectDefinition, { readonly type: "modify-combat-outcome" }>,
      move,
      context,
      target,
      effectIndex,
  ...
  "set-roll-result": (effect, move, context, target, _trigger, effectIndex) =>
  ...
  "prevent-low-roll-stop": (effect, _move, context, target) =>
    preventLowRollStopEffectChanges(
      effect as Extract<EffectDefinition, { readonly type: "prevent-low-roll-stop" }>,
      context,
      target,
    ),
  ...
  "override-resolution-immunity": (effect, _move, _context, target) =>
    resolutionImmunityOverrideEffectChanges(
      effect as Extract<EffectDefinition, { readonly type: "override-resolution-immunity" }>,
      target,
    ),
  "suppress-requirement": (
    ...[effect, , context, target, , effectIndex]: Parameters<TriggeredEffectHandler>
  ) =>
    suppressRequirementEffectChanges(
      effect as Extract<EffectDefinition, { readonly type: "suppress-requirement" }>,
      context,
      target,
      effectIndex,
    ),
  "modify-damage-reduction-cost": (
    ...
    damageReductionCostEffectChanges(
      effect as Extract<EffectDefinition, { readonly type: "modify-damage-reduction-cost" }>,
      move,
      context,
      target,
      effectIndex,
  ...
  "resolve-contest": (
    ...
    contestEffectChanges(
      effect as Extract<EffectDefinition, { readonly type: "resolve-contest" }>,
      move,
      context,
      target,
      effectIndex,
  ...
  "require-transformation-roll": (
    ...
    transformationRollRequirementEffectChanges(
      effect as Extract<EffectDefinition, { readonly type: "require-transformation-roll" }>,
      move,
      target,
      effectIndex,
    ),
  ...
  "grant-destruction-mastery": (
    ...[effect, move, , , , effectIndex]: Parameters<TriggeredEffectHandler>
  ) =>
    destructionMasteryEffectChanges(
      effect as Extract<EffectDefinition, { readonly type: "grant-destruction-mastery" }>,
      move,
      effectIndex,
    ),
  ...
  negate: (...[effect, move, context, target, , effectIndex]: Parameters<TriggeredEffectHandler>) =>
    negateEffectChanges(
  ...
  "negate-deactivation": (
    ...
    const typedEffect = effect as Extract<
      EffectDefinition,
      { readonly type: "negate-deactivation" }
    >;
    let count: number | undefined;
    if (typedEffect.useLimit?.scope === "combat") {
      count =
        typeof typedEffect.useLimit.count === "number"
          ? typedEffect.useLimit.count
          : numeric(typedEffect.useLimit.count, context);
    }
    return {
      ...emptyEffectChanges(),
      deactivationNegations:
        count === undefined
          ? []
          : [
              {
                target,
                optional: typedEffect.optional === true,
                selector: typedEffect.selector,
                sourceDefinitionId: move.id,
                sourceEffectIndex: effectIndex,
                useLimit: { scope: "combat" as const, count },
              },
  ...
  reroll: (...[effect, move, context, target, , effectIndex]: Parameters<TriggeredEffectHandler>) =>
    rerollEffectChanges(
  ...
  "revert-transformation": (
    ...
    transformationReversionEffectChanges(
      effect as Extract<EffectDefinition, { readonly type: "revert-transformation" }>,
      move,
      context,
      target,
      effectIndex,
  ...
  "set-stat-comparison": (
    ...
    statComparisonEffectChanges(
      effect as Extract<EffectDefinition, { readonly type: "set-stat-comparison" }>,
      move,
      context,
      target,
      effectIndex,
  ...
  "grant-transformation-action": (
    ...
    transformationActionEffectChanges(
      effect as Extract<EffectDefinition, { readonly type: "grant-transformation-action" }>,
      move,
      target,
      effectIndex,
    ),
  "reactivate-recent-skill": (
    ...
    reactivationEffectChanges(
      effect as Extract<EffectDefinition, { readonly type: "reactivate-recent-skill" }>,
      move,
      context.self.id,
      target,
      effectIndex,
  ...
  "activate-protected-constant": (
    ...
    protectedActivationEffectChanges(
      effect as Extract<EffectDefinition, { readonly type: "activate-protected-constant" }>,
      move,
      context.self.id,
      target,
      trigger,
  ...
  "force-transformation": (
    ...
    forceTransformationEffectChanges(
      effect as Extract<EffectDefinition, { readonly type: "force-transformation" }>,
      move,
      target,
      effectIndex,
    ),
...
const triggeredEffectChanges = (
  ...[effect, move, context, trigger, target, effectIndex]: [
    effect: EffectDefinition,
    move: MoveDefinition,
    context: MoveEffectRuntimeContext,
    trigger:
  ...
  const groupedChoiceEnabled =
    effect.activationGroup !== undefined &&
    context.enabledOptionalEffectIndices?.some((candidateIndex) => {
      const candidate = move.effects?.[candidateIndex];
      return (
        candidateIndex !== effectIndex && candidate?.activationGroup === effect.activationGroup
      );
    }) === true;
  if (
    ...
    (effect.optional === true &&
      ...
      !(
        ...
        (((trigger === "on-success" ||
          ...
          (effect.type === "activate" || effect.type === "activate-protected-constant")) ||
          ((trigger === "upkeep-phase" || trigger === "turn-end") &&
            effect.type === "deactivate") ||
          (trigger === "on-deactivated" &&
            effect.type === "negate-deactivation" &&
            context.triggeringMove !== undefined))
      )) ||
    (effect.activationGroup !== undefined &&
      effect.type !== "reroll" &&
      !context.enabledOptionalEffectIndices?.includes(effectIndex) &&
      !groupedChoiceEnabled &&
      !(
        context.collectPendingChoices === true &&
...
export interface MoveClassificationApplication {
  readonly target: "self" | "opponent";
  readonly sourceDefinitionId: MoveDefinition["id"];
  readonly effectIndex: number;
  readonly selector: MoveSelectorCondition;
  readonly classification:
    "replace-declared-style" | { readonly type: "replace-style-id"; readonly styleId: string };
  readonly duration:
    { readonly type: "turns"; readonly turns: number } | { readonly type: "combat" };
}
...
export const classifyCurrentActionMove = (
  ...
  const durableMove = (context.activeEffects ?? []).reduce((current, activeEffect) => {
    if (
      activeEffect.type !== "modify-move-classification" ||
      activeEffect.targetCombatantId !== context.self.id ||
      runtimeValue(activeEffect.classification.type) !== "replace-style" ||
      !matchesMoveSelector(current, activeEffect.selector) ||
      (activeEffect.classification.style === "declared-style" &&
        context.self.declaredStyleId === undefined)
    )
      return current;
    return {
      ...current,
      styleId:
        activeEffect.classification.style === "declared-style"
          ? context.self.declaredStyleId
          : activeEffect.classification.styleId,
    };
  }, move);
  ...
  if (uniqueAddedTags.length === 0) return { move: durableMove, addedTags: [] };
  return {
    move: { ...durableMove, tags: [...new Set([...durableMove.tags, ...uniqueAddedTags])] },
...
// eslint-disable-next-line complexity -- Effect translation intentionally keeps related declarative branches together.
const isSupportedUpkeepSuppressionChoice = (effect: EffectDefinition) => {
  if (
    effect.type !== "suppress" ||
    ...
    return false;
  const durationTurns =
    effect.duration.turns.type === "literal" ? effect.duration.turns.value : undefined;
  let useLimitCount: number | undefined;
  if (typeof effect.useLimit.count === "number") useLimitCount = effect.useLimit.count;
  else if (effect.useLimit.count.type === "literal") useLimitCount = effect.useLimit.count.value;
  const activationCostAmount =
    effect.activationCost?.amount.type === "literal"
      ? effect.activationCost.amount.value
      : undefined;
  const showdown =
    effect.target === "participants" &&
    ...
    activationCostAmount === 1;
  const againstTheOdds =
    effect.target === "opponent" &&
    effect.selector.category === "advanced-attack" &&
    durationTurns === 3 &&
    useLimitCount === 2 &&
    effect.activationCost === undefined;
  return showdown || againstTheOdds;
};

const isEndFloatingEffectChoice = (effect: EffectDefinition) =>
...
const moveEffectsForTriggerInternal = (
  ...
  readonly endFloatingEffects: readonly EndFloatingEffectApplication[];
  ...
  readonly moveEffectReplacements?: readonly MoveEffectReplacementApplication[];
  ...
  readonly resolutionImmunityOverrides: readonly ResolutionImmunityOverrideApplication[];
  readonly damageReductionCostModifications: readonly DamageReductionCostModificationApplication[];
  readonly contests: readonly ContestApplication[];
  readonly destructionMasteries: readonly DestructionMasteryApplication[];
  readonly transformationRollRequirements: readonly TransformationRollRequirementApplication[];
  readonly combatResultOverrides: readonly CombatResultOverrideApplication[];
  readonly combatOutcomeModifiers: readonly CombatOutcomeModifierApplication[];
  ...
  readonly moveClassifications: readonly MoveClassificationApplication[];
  readonly rerolls: readonly RerollApplication[];
  readonly negations: readonly NegationApplication[];
  readonly deactivationNegations?: readonly DeactivationNegationApplication[];
  readonly transformationReversions: readonly TransformationReversionApplication[];
  readonly statComparisons: readonly StatComparisonApplication[];
  ...
  // eslint-disable-next-line complexity, max-lines-per-function, max-statements, sonarjs/cognitive-complexity -- Effect translation intentionally keeps related declarative branches together.
} => {
  ...
  const damageModifierTransformers: DamageModifierTransformerApplication[] = [];
  const resourceModifierTransformers: ResourceModifierTransformerApplication[] = [];
  const costModifierTransformers: CostModifierTransformerApplication[] = [];
  ...
  const deferredMoves: DeferredMoveApplication[] = [];
  ...
  const resolutionImmunityOverrides: ResolutionImmunityOverrideApplication[] = [];
  const damageReductionCostModifications: DamageReductionCostModificationApplication[] = [];
  const contests: ContestApplication[] = [];
  const destructionMasteries: DestructionMasteryApplication[] = [];
  const transformationRollRequirements: TransformationRollRequirementApplication[] = [];
  const combatResultOverrides: CombatResultOverrideApplication[] = [];
  const combatOutcomeModifiers: CombatOutcomeModifierApplication[] = [];
  ...
  const moveClassifications: MoveClassificationApplication[] = [];
  const rerolls: RerollApplication[] = [];
  const negations: NegationApplication[] = [];
  const deactivationNegations: DeactivationNegationApplication[] = [];
  const transformationReversions: TransformationReversionApplication[] = [];
  const statComparisons: StatComparisonApplication[] = [];
  const transformationActions: TransformationActionApplication[] = [];
  const pendingEffectChoices: PendingEffectChoice[] = [];
  let requireAllDiceSuccess = false;
  ...
  if (
    ...
    for (const [effectIndex, effect] of (move.effects ?? []).entries()) {
      if ((trigger === "upkeep-phase" || trigger === "turn-end") && effect.type === "deactivate")
        continue;
      if (
        effect.trigger !== trigger ||
        (effect.optional !== true &&
          ...
          !(trigger === "action-phase" && effect.type === "exchange-constant-skill") &&
          !isSupportedUpkeepSuppressionChoice(effect) &&
    ...
    for (const [key, effectIndices] of choiceGroups) {
      const supportedOnSuccessChoice =
        ...
        (() => {
          ...
          const spinebreakerChoiceGroup =
            ...
            effects.some((effect) => effect.type === "apply-status") &&
            effects.some((effect) => effect.type === "modify-roll");
          const protectedActivationChoiceGroup =
            effects.length === 1 && effects[0]?.type === "activate-protected-constant";
          const replaceActiveConstantChoiceGroup =
            effects.length === 1 && effects[0]?.type === "replace-active-constant-effects";
          const endFloatingChoiceGroup =
            effects.length === 1 &&
            runtimeValue(effects[0]) !== undefined &&
            isEndFloatingEffectChoice(effects[0]);
          const spikedBallChoiceGroup =
            effects.length === 2 &&
            effects.some(
              (effect) =>
                effect.type === "modify-remaining-uses" &&
            ...
            effects.some(
              (effect) =>
                effect.type === "replace-move-effect" &&
                effect.target === "self" &&
          ...
          return (
            ...
            protectedActivationChoiceGroup ||
            replaceActiveConstantChoiceGroup ||
            spikedBallChoiceGroup ||
            endFloatingChoiceGroup
          );
        })();
      if (!supportedOnSuccessChoice) continue;
      const supportedPassiveChoice =
        trigger !== "passive" ||
      ...
      if (!supportedPassiveChoice) continue;
      ...
      const supportedBeforeDefenseChoice =
        ...
        (effectIndices.length === 3 &&
          // eslint-disable-next-line complexity -- Effect translation intentionally keeps related declarative branches together.
          effectIndices.every((effectIndex) => {
            const effect = move.effects![effectIndex];
            return (
              (effect.type === "grant-counter-action" &&
                effect.target === "self" &&
                effect.action === "use-source-attack" &&
                effect.stopsTriggeringAttack === false) ||
              (effect.type === "set-roll-result" &&
                effect.target === "self" &&
                effect.roll === "defense" &&
          ...
          effectIndices.some(
            (effectIndex) => move.effects![effectIndex].type === "grant-counter-action",
          ) &&
          effectIndices.some(
            (effectIndex) => move.effects![effectIndex].type === "set-roll-result",
          ) &&
          effectIndices.some(
            (effectIndex) => move.effects![effectIndex].type === "modify-damage",
          ) &&
          effectIndices.some(
            (effectIndex) => move.effects![effectIndex].type === "modify-damage",
          )) ||
        (effectIndices.length === 3 &&
          ...
          effectIndices.every((effectIndex) => {
            ...
            const beamCondition =
              ...
              condition.categoryExcludes?.includes("signature") === true &&
              condition.tags?.includes("beam") === true;
            const sacrificeRemoval =
            ...
            const beamLock =
              effect.type === "lock" &&
              effect.target === "opponent" &&
              effect.affectedType === "attack" &&
              effect.selector?.subject === "target" &&
              effect.selector.category === "advanced-attack" &&
              effect.selector.categoryExcludes?.includes("signature") === true &&
              effect.selector.tags?.includes("beam") === true &&
              effect.duration?.type === "turns" &&
              effect.duration.turns.type === "literal" &&
              effect.duration.turns.value === 4;
            return sacrificeRemoval || damageNegation || beamLock;
          }) &&
          effectIndices.some(
            (effectIndex) => move.effects![effectIndex].type === "remove-move-from-combat",
          ) &&
          effectIndices.some((effectIndex) => move.effects![effectIndex].type === "negate") &&
          effectIndices.some((effectIndex) => move.effects![effectIndex].type === "lock")) ||
        (effectIndices.length === 1 &&
          // eslint-disable-next-line complexity -- Effect translation intentionally keeps related declarative branches together.
          effectIndices.every((effectIndex) => {
            const effect = move.effects![effectIndex];
      ...
      const supportedRollModifiedChoice =
        trigger !== "on-roll-modified" ||
        effectIndices.every((effectIndex) => {
          const effect = move.effects![effectIndex];
          return (
      ...
      if (!supportedRollModifiedChoice) continue;
      ...
      const supportedOnMoveUseDamageChoice =
        trigger !== "on-move-use" ||
        (effectIndices.length === 2 &&
          // eslint-disable-next-line complexity -- Effect translation intentionally keeps related declarative branches together.
          effectIndices.every((effectIndex) => {
            const effect = move.effects![effectIndex];
            return (
          ...
          effectIndices.some(
            (effectIndex) => move.effects![effectIndex].type === "prevent-resolution",
          ));
      const supportedOnMoveUseNegationChoice =
        trigger !== "on-move-use" ||
        (effectIndices.length === 1 &&
          effectIndices.every((effectIndex) => {
            const effect = move.effects![effectIndex];
      ...
      const supportedLifecycleChoice =
        (trigger !== "upkeep-phase" && trigger !== "turn-end") ||
        effectIndices.every((effectIndex) => {
          const effect = move.effects![effectIndex];
          return (
      ...
      const supportedActionPhaseExchangeChoice =
        trigger === "action-phase" &&
        effectIndices.length === 1 &&
        effectIndices.every(
          (effectIndex) =>
            move.effects![effectIndex].type === "exchange-constant-skill" &&
            move.effects![effectIndex].optional === undefined,
        );
      const supportedUpkeepFloatingChoice =
        trigger === "upkeep-phase" &&
        effectIndices.length === 1 &&
        effectIndices.every((effectIndex) => {
          const effect = move.effects![effectIndex];
      ...
      const supportedUpkeepSuppressionChoice =
        trigger === "upkeep-phase" &&
        effectIndices.length === 1 &&
        effectIndices.every((effectIndex) =>
      ...
      const combinedOnMoveUseDamageChoice =
        trigger === "on-move-use" &&
        effectIndices.length === 2 &&
        effectIndices.some((effectIndex) => move.effects![effectIndex].type === "modify-damage") &&
        effectIndices.some(
          (effectIndex) => move.effects![effectIndex].type === "prevent-resolution",
        );
      const alternatives =
  ...
  for (const [effectIndex, effect] of (move.effects ?? []).entries()) {
    const isStartCombatSelectionEffect =
      trigger === "start-combat" &&
      effect.type === "modify-remaining-uses" &&
      effect.selector.subject === "source" &&
      effect.selector.ids === undefined;
    ...
    const groupedChoiceEnabled =
      ...
      (move.effects ?? []).some(
        (candidate, candidateIndex) =>
          ...
          candidate.activationGroup === effect.activationGroup &&
          enabledEffectIndices.has(candidateIndex),
      );
    const collectActivationChoice =
      context.collectPendingChoices === true &&
      (trigger === "on-success" || trigger === "start-combat" || trigger === "on-roll-result") &&
      effect.type === "activate";
    const collectLifecycleDeactivationChoice =
      context.collectPendingChoices === true &&
      (trigger === "upkeep-phase" || trigger === "turn-end") &&
      effect.type === "deactivate";
    const collectActionPhaseExchangeChoice =
      context.collectPendingChoices === true &&
      trigger === "action-phase" &&
      effect.type === "exchange-constant-skill";
    if (
      isChoiceEffect &&
      !enabledEffectIndices.has(effectIndex) &&
      !groupedChoiceEnabled &&
      !collectActivationChoice &&
      !collectLifecycleDeactivationChoice &&
    ...
    const compiled = compileEffectPlan({
      ...
      allowPendingChoice:
        (isChoiceEffect && (enabledEffectIndices.has(effectIndex) || groupedChoiceEnabled)) ||
        collectActivationChoice ||
        collectLifecycleDeactivationChoice ||
        collectActionPhaseExchangeChoice,
    });
    ...
    for (const target of effectTargets(compiled.value.definition)) {
      ...
      requireAllDiceSuccess ||= changes.requireAllDiceSuccess;
      ...
      damageModifierTransformers.push(
        ...(runtimeOptional(changes.damageModifierTransformers) ?? []),
      );
      resourceModifierTransformers.push(
        ...(runtimeOptional(changes.resourceModifierTransformers) ?? []),
      );
      costModifierTransformers.push(...(runtimeOptional(changes.costModifierTransformers) ?? []));
      ...
      deferredMoves.push(...(runtimeOptional(changes.deferredMoves) ?? []));
      ...
      moveRemovals.push(...(runtimeOptional(changes.moveRemovals) ?? []));
      ...
      resolutionImmunityOverrides.push(...changes.resolutionImmunityOverrides);
      damageReductionCostModifications.push(...changes.damageReductionCostModifications);
      contests.push(...changes.contests);
      destructionMasteries.push(...changes.destructionMasteries);
      transformationRollRequirements.push(...changes.transformationRollRequirements);
      ...
      combatOutcomeModifiers.push(...changes.combatOutcomeModifiers);
      ...
      moveClassifications.push(...changes.moveClassifications);
      rerolls.push(...changes.rerolls);
      negations.push(...changes.negations);
      deactivationNegations.push(...changes.deactivationNegations);
      transformationReversions.push(...changes.transformationReversions);
      statComparisons.push(...changes.statComparisons);
      transformationActions.push(...changes.transformationActions);
    }
  }
  if (includeActiveFloatingEffects) {
    for (const floating of context.activeEffects ?? []) {
      ...
      resourceActionModifiers.push(...(runtimeOptional(nested.resourceActionModifiers) ?? []));
      resourceCostModifications.push(...(runtimeOptional(nested.resourceCostModifications) ?? []));
      damageModifierTransformers.push(
        ...(runtimeOptional(nested.damageModifierTransformers) ?? []),
      );
      resourceModifierTransformers.push(
        ...(runtimeOptional(nested.resourceModifierTransformers) ?? []),
      );
      costModifierTransformers.push(...(runtimeOptional(nested.costModifierTransformers) ?? []));
      ...
      deferredMoves.push(...(runtimeOptional(nested.deferredMoves) ?? []));
      ...
      moveRemovals.push(...(runtimeOptional(nested.moveRemovals) ?? []));
      ...
      resolutionImmunityOverrides.push(...nested.resolutionImmunityOverrides);
      damageReductionCostModifications.push(...nested.damageReductionCostModifications);
      contests.push(...nested.contests);
      destructionMasteries.push(...nested.destructionMasteries);
      transformationRollRequirements.push(...nested.transformationRollRequirements);
      combatResultOverrides.push(...nested.combatResultOverrides);
      combatOutcomeModifiers.push(...nested.combatOutcomeModifiers);
      ...
      moveClassifications.push(...nested.moveClassifications);
      rerolls.push(...nested.rerolls);
      negations.push(...nested.negations);
      deactivationNegations.push(...(nested.deactivationNegations ?? []));
      transformationReversions.push(...nested.transformationReversions);
      statComparisons.push(...nested.statComparisons);
      transformationActions.push(...nested.transformationActions);
  ...
  return {
    ...
    requireAllDiceSuccess,
  };
};

const moveWithActiveEffectReplacements = (
  move: MoveDefinition,
  context: MoveEffectRuntimeContext,
): MoveDefinition => {
  ...
  if (replacements.length === 0 || move.effects === undefined) return move;
  const effects = [...move.effects];
  for (const replacement of replacements) {
    effects[replacement.sourceEffectIndex] = {
...
export const moveEffectsForTrigger = (
  ...
  moveEffectsForTriggerInternal(
    moveWithActiveEffectReplacements(move, context),
    trigger,
    context,
...
export const successfulMoveEffects = (move: MoveDefinition, context: MoveEffectRuntimeContext) =>
  context.successfulEffectsRequireAllDice &&
  (context.rolls === undefined ||
    context.rolls.length === 0 ||
    context.rolls.some((roll) => roll.outcome !== "successful"))
    ? { ...emptyEffectChanges(), pendingEffectChoices: [] }
